...
def test_run_indexing_builds_text_and_visual_entries_with_injected_dependencies(tmp_path):
    ...
    actual_metadatas = collection.upsert_call["metadatas"]
    # context_hash is content-derived and not worth recomputing in the test;
    # assert it exists and is a non-empty hex string, then compare the rest.
    for metadata in actual_metadatas:
        assert isinstance(metadata.pop("context_hash"), str)
    assert actual_metadatas == [
        {
            "title": "Arrival",
            "summary": "person enters",
            "speakers": "Alice, Bob",
            "speakers_tokens": "|alice|bob|",
            "keywords": "arrival",
            "keywords_tokens": "|arrival|",
            "actions": "walking",
            "actions_tokens": "|walking|",
            "audio_events": "announcement, crowd",
            "audio_events_tokens": "|announcement|crowd|",
            "start_time": 1.5,
            "end_time": 4.0,
            "duration_sec": 2.5,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([{"segment_id": "segment-1", "start_time": "0", "end_time": 1}], "start_time"),
        ...
        ([{"segment_id": "segment-1", "start_time": 0, "end_time": "1"}], "end_time"),
